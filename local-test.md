...
## 8. Режим личного чата с ботом

Теперь можно писать боту в личку (private chat).

- Используется одна и та же привязка на пользователя: synthetic `thread_id=0`
- Привязка идёт к одному окну с именем `TMUX_SESSION_NAME` из `.env` (например `ccbot`)
- Если окна нет, оно создаётся автоматически в `Path.cwd()` при первом сообщении

Проверка:

1. Запустить `uv run ccbot`
2. Открыть личный чат с ботом
3. Отправить обычный текст (не `/start`)
4. Проверить, что в tmux появился/используется window `ccbot`
5. Отправить второе сообщение и убедиться, что оно попадает в ту же сессию

## 9. Быстрый smoke-check сценарий
